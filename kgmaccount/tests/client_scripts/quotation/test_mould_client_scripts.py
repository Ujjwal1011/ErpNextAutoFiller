"""Tests for Quotation mould and MouldG Client Scripts.
...
How it checks:
- Runs `Quotation Moulding Dialog Box` to verify mould rows copy height and
  width from the previous stone row.
- Runs `Qutotation Kota Granite Moulding Calculation` to calculate running
  feet for Mould and MouldG rows. The name keeps the fixture's existing
  spelling so the test targets the exported script exactly.
...
- If the all-row mould test fails, failed rows are written to
  `kgmaccount/tests/logs/quotation_mould_failed_rows.json`.
"""
...
class TestQuotationMouldClientScripts(unittest.TestCase):
    ...
    def test_mould_dialog_copies_previous_item_dimensions(self):
        """Quotation mould dialog should copy height/width from the previous stone row."""
        ...
        self.assertEqual(result["dialog_title"], "Item Custom Dimensions", debug)
    ...
    def test_job_work_dialog_selects_all_sides_by_default(self):
        """Quotation job work rows should open the mould dialog with all sides selected."""
        result = run_item_client_script(
            PROFILE,
            CLIENT_SCRIPT_PROFILES[PROFILE]["mould_dialog_script"],
            "item_code",
    ...
    def test_job_work_quantity_calculation_matches_csv_running_feet_with_all_sides(self):
        """Quotation job work rows use moulding running-foot calculation instead of sqft."""
    ...
    def _run_mould_case(self, case, side_values):
        """Run the Quotation mould calculation script for one CSV-backed item."""
        row = {
    ...
    def _assert_all_mould_rows_can_match_running_feet(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} mould CSV rows could not match any Quotation side selection. "
