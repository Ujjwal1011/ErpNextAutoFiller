"""Separate Quotation Select Item tests for Kaddpa rows.
...
- Sends the CSV `Height` and `Width` into the fake Quotation Item row.
- Sets the Select Item dialog template option to `KADDPA`.

How it checks:
- Runs `Quotation Select Item` in Node.js with the fake Frappe dialog.
...
- Failed Kaddpa rows are written to
  `kgmaccount/tests/logs/select_item/quotation_kaddpa_select_item_failed_rows.csv`.
"""
...
PROFILE = "quotation"
LOG_FILENAME = "quotation_kaddpa_select_item_failed_rows.json"


class TestQuotationKaddpaSelectItemDialog(unittest.TestCase):
    def test_all_kaddpa_csv_rows_generate_expected_item_code_and_cut_size(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} Kaddpa CSV rows do not match the Quotation Select Item dialog. "
