"""Tests for the Sales Invoice sqft calculation Client Script.
...
How it checks:
- Runs `Sales-Invoice Kota Kaddpaa Granite Neno Calculation` in Node.js.
- Compares Client Script `qty` with CSV `SQFT`.
- Focused branch tests also compare cut sizes with CSV `Cut From Height` and
  `Cut From Width`, and check CSV `Rate`.
- Covers Raj Kota, Kota without Raj, without Kota/default granite, and Kaddpa.

Logs:
- If the all-row CSV test fails, failed rows are written to
  `kgmaccount/tests/logs/sales_invoice_sqft_failed_rows.json`.
"""
...
class TestSalesInvoiceSqftCalculationClientScript(unittest.TestCase):
    def test_all_applicable_csv_rows_match_sqft_client_script(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} CSV rows do not match the current Sales Invoice sqft Client Script. "
