"""Tests for the Sales Order sqft calculation Client Script.
...
- If the all-row CSV test fails, failed rows are written to
  `kgmaccount/tests/logs/sales_order_sqft_failed_rows.json`.
"""
...
class TestSalesOrderSqftCalculationClientScript(unittest.TestCase):
    def test_all_applicable_csv_rows_match_sqft_client_script(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} CSV rows do not match the current Sales Order sqft Client Script. "
