"""Tests for the Sales Order `custom_select_item` dialog Client Script.

Input:
- Reads selected CSV cases from
  `/workspace/development/frappe-bench/apps/kgmaccount/sales order item list.csv`.
- Sends CSV `Height` and `Width` to the fake Sales Order Item row.
...
How it checks:
- Runs `Sales-Order Select Item` in Node.js with a fake `frappe.ui.Dialog`.
...
- If any Kota CSV row does not match, failed rows are written to
  `kgmaccount/tests/logs/sales_order_select_item_failed_rows.csv`.
"""
...
class TestSalesOrderSelectItemDialog(unittest.TestCase):
    def test_all_kota_csv_rows_generate_expected_item_code_and_cut_size(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} Kota CSV rows do not match the Sales Order Select Item dialog. "
                f"Failed rows logged at: {log_path}\n"
                f"Showing first 25:\n{preview}"
            )

    def test_raj_kota_dialog_generates_item_code_and_cut_size(self):
        """Raj Kota: dialog should generate `KOTA 24x24 RAJ G` and cut size."""
    ...
    def test_kota_without_raj_dialog_generates_item_code_and_cut_size(self):
        """Kota without Raj: dialog should generate `KOTA 42x24` and cut size."""
    ...
    def test_kaddpa_dialog_generates_item_code_and_cut_size(self):
        """Kaddpa: dialog template switch should generate `KADDPA 42x24`."""
