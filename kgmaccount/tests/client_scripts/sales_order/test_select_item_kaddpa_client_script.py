"""Separate Sales Order Select Item tests for Kaddpa rows.
...
- Sends the CSV `Height` and `Width` into the fake Sales Order Item row.
- Sets the Select Item dialog template option to `KADDPA`.

How it checks:
- Runs `Sales-Order Select Item` in Node.js with the fake Frappe dialog.
...
- Failed Kaddpa rows are written to
  `kgmaccount/tests/logs/select_item/sales_order_kaddpa_select_item_failed_rows.csv`.
"""
...
PROFILE = "sales_order"
LOG_FILENAME = "sales_order_kaddpa_select_item_failed_rows.json"


class TestSalesOrderKaddpaSelectItemDialog(unittest.TestCase):
    def test_all_kaddpa_csv_rows_generate_expected_item_code_and_cut_size(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} Kaddpa CSV rows do not match the Sales Order Select Item dialog. "
