"""Tests for Sales Order mould and MouldG Client Scripts.
...
How it checks:
- Runs `Sales-Order Moulding Dialog Box` to verify mould rows copy height and
  width from the previous stone row.
- Runs `Sales-Order Kota Granite Moulding Calculation` to calculate running
...
- If the all-row mould test fails, failed rows are written to
  `kgmaccount/tests/logs/sales_order_mould_failed_rows.json`.
"""
...
class TestSalesOrderMouldClientScripts(unittest.TestCase):
    ...
    def test_mould_dialog_copies_previous_item_dimensions(self):
        """Mould dialog should copy height/width from the previous stone item row."""
    ...
    def _assert_all_mould_rows_can_match_running_feet(self):
        ...
        if mismatches:
            ...
            self.fail(
                f"{len(mismatches)} mould CSV rows could not match any Sales Order side selection. "
